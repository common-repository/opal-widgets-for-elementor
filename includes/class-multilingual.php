...
class OSF_Multilingual {
	...
	public function get_original_object( $id, $type = 'post' ) {
		return wpml_object_id_filter( $id, $type, true, $this->get_default_language() );
	}

	/**
	 * Get the current object ID (post, taxonomy, etc...).
	 *
	 * @param  int    $id   The object id.
	 * @param  string $type Optional, post type or taxonomy name of the object, defaults to 'post'.
	 * @return int|null
	 */
	public function get_current_object( $id, $type = 'post' ) {
		return wpml_object_id_filter( $id, $type, true, $this->get_current_language() );
	}
